...
module.exports = function loadTypes(basepath, fn) {
  ...
  // read default lib resources from lib/ directory (Dashboard, ClientLib)
  ['dashboard.js', 'clientlib.js'].forEach(function(file) {
    try {
      var customResource = require(__dirname + '/' + file);
      if (customResource && customResource.name) {
        defaults[customResource.name] = customResource;
        debug('Loaded native resource:', customResource.name);
      }
    } catch (e) {
      debug('Could not load', file, e.message);
    }
  });

  // read default lib resources from lib/resources/
  fs.readdir(__dirname + '/resources', function(err, dir) {
    ...
    if (fs.existsSync(packageJsonPath)) {
      ...
      if (packageJson && (packageJson.dependencies || packageJson.devDependencies)) {
        ...
        // If no custom resources were loaded (all ignored or no dependencies), call callback
        if (remaining === 0) {
          fn(defaults, types);
        }
      } else {
