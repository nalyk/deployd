...
exports.setup = function(options, req, res, next) {
  ...
  handler(req, res, function () {
    ...
    // Request ID tracking for distributed tracing and log correlation
    // Accept existing X-Request-ID header or generate new UUID
    req.id = req.headers['x-request-id'] || uuid.v4();
    res.setHeader('X-Request-ID', req.id);

    // Store options on request for use in getBody
    req._deploydOptions = options;
...
exports.getBody = function(req, callback){
  ...
  // Configurable request body size limit (default 1MB)
  // Can be set via server options: {maxBodySize: 5 * 1024 * 1024} for 5MB
  var options = req._deploydOptions || {};
  var maxBodySize = options.maxBodySize || (1024 * 1024); // Default 1MB
  var receivedBytes = 0;

  req.on('data', function(chunk){
    receivedBytes += chunk.length;

    // Security: Enforce request body size limit to prevent DoS attacks
    if (receivedBytes > maxBodySize) {
      req.removeAllListeners('data');
      req.removeAllListeners('end');

      // Send 413 Payload Too Large
      var maxMB = (maxBodySize / (1024 * 1024)).toFixed(1);
      var error = new Error('Request body too large (max ' + maxMB + 'MB)');
...
var parseBody = exports.parseBody = function(req, res, mime, callback) {
  exports.getBody(req, function(buf){
    ...
    if (buf instanceof Error) {
      ...
      res.statusCode = buf.statusCode || 413;
      res.end(JSON.stringify({error: buf.message}));
      return;
    }
