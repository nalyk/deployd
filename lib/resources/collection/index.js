...
Collection.prototype.sanitizeQuery = function (query) {
  ...
  // Security: Block dangerous operators that execute code on MongoDB server
  // These operators can run arbitrary JavaScript and are blocked for security
  // All other MongoDB operators are allowed (deployd supports all MongoDB query operators)
  var DANGEROUS_OPERATORS = [
    '$where',        // Executes JavaScript on MongoDB server
    '$function',     // Executes JavaScript (aggregation)
  ...
  keys && keys.forEach(function (key) {
    ...
    // Security check: Block dangerous operators only
    if (DANGEROUS_OPERATORS.indexOf(key) !== -1) {
...
// Integrate dpd-count: adds /count endpoint to collections
// Native integration of https://github.com/docnoe/dpd-count
(function() {
  /**
   * Creates a domain object for event scripts
   */
  ...
  /**
   * Count endpoint handler - returns {count: N}
   */
  Collection.prototype.count = function(ctx, fn) {
    var collection = this;
    var query = ctx.query || {};
    var errors = {};
    var domain = createDomain(query, errors);

    var goOn = function(err, res) {
      if (err) {
        return fn(err);
      }

      var store = collection.store;
      var sanitizedQuery = collection.sanitizeQuery(ctx.query || {});
