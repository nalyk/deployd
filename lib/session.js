...
function SessionStore(namespace, db, sockets, options) {
  ...
  // Defer cleanup to avoid connection race conditions on startup
  this._cleanupDeferred = true;
  if (db) {
    // Cleanup inactive sessions from the db, but delay it to avoid
    // racing with the initial connection (TLS handshake can take time)
    var store = this;
    setTimeout(function() {
      store._cleanupDeferred = false;
      store.cleanupInactiveSessions();
    }, 5000); // 5 second delay to ensure MongoDB connection is established
  }
}
...
SessionStore.prototype.cleanupInactiveSessions = function() {
  ...
  // Check if database connection is ready before attempting cleanup
  if (!this.db || !this.db.Db) {
    // Connection not ready yet, skip this cleanup cycle
  ...
  // Wrap in try-catch to prevent cleanup failures from crashing the app
  try {
    this.remove({
      $or: [
      ...
      if (err) {
        ...
        // The cleanup will retry on the next interval
        error("Session cleanup failed (will retry): " + err.message);
      }
    });
  } catch (err) {
    // Exception during cleanup - not critical, will retry later
    error("Exception during session cleanup (will retry): " + err.message);
  }
  this.cleanupInactiveSessions.lastRun = Date.now();

  // Schedule next cleanup in 1 minute if not already scheduled
...
function Session(data, store, sockets, rawSockets) {
  ...
  this._leaveAllRooms = function(except, fn) {
    ...
    async.forEachOf(self.store.socketIndex[self.sid], function(socket, id, outer) {
      // Socket.IO v4: socket.rooms is now a Set, convert to array
      var roomsArray = socket.rooms instanceof Set ? Array.from(socket.rooms) : socket.rooms;
      async.each(_.difference(_.without(roomsArray, socket.id, userChannel), except), function(room, inner) {
