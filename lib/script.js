...
Script.prototype.run = function (ctx, domain, fn) {
  ...
  var req = ctx.req
    ...
    , scriptTimeout = (ctx.server && ctx.server.options && ctx.server.options.scriptTimeout) || 10000
    , timeoutTimer = null
    , timedOut = false;
  ...
  function done(err) {
    ...
    var executionTime = Date.now() - scriptStartTime;
    if (executionTime > 1000) {
      debug('SLOW SCRIPT: %s took %dms', ctx.url || 'unknown', executionTime);
    }
  ...
  // Set up timeout timer to prevent runaway scripts
  // This is a soft timeout - it won't interrupt synchronous infinite loops,
  // but will prevent async operations from hanging indefinitely
  timeoutTimer = setTimeout(function() {
    if (!isDone) {
      timedOut = true;
      var timeoutError = new Error('Script execution timeout after ' + scriptTimeout + 'ms');
      timeoutError.statusCode = 500;
      debug('SCRIPT TIMEOUT: %s exceeded %dms', ctx.url || 'unknown', scriptTimeout);
      done(timeoutError);
    }
  }, scriptTimeout);
  ...
  try {
    ...
    var syncExecutionTime = Date.now() - syncStartTime;

    // Warn about long synchronous execution (potential infinite loop)
    if (syncExecutionTime > 500) {
      debug('WARNING: Synchronous script execution took %dms at %s', syncExecutionTime, ctx.url || 'unknown');
    }
  } catch(e) {
    err = wrapError(e);
...
function wrapAsyncFunction(asyncFunction, sandbox, events, done, sandboxRoot) {
  return function() {
    ...
    if (typeof callback === 'function') {
      events.emit('addCallback');
      var callbackStartTime = Date.now();
      args[callbackIndex] = function() {
        if (sandboxRoot._error) return;
        try {
          result = callback.apply(sandboxRoot._this, arguments);
          var callbackDuration = Date.now() - callbackStartTime;
          if (callbackDuration > 500) {
            debug('SLOW CALLBACK: took %dms', callbackDuration);
          }
          events.emit('finishCallback');
        } catch (err) {
